...
**`/feature` is the inner loop; `/review` is the outer one.** Review's unit is the change set, not the slice: it scores the change's risk, fans `.speccle/lenses/` over the diff, fixes only what that score permits, and lands one commit. It pushes on the pull-request route below and nowhere else, and only once that commit has landed. On a pull request the same lenses run in CI (`.github/workflows/speccle-review.yml`, metered key) and `/address` acts on what they posted rather than deriving findings again. The checks-gate each implement session runs includes `.speccle/checks/` — this repo's own deterministic invariants, each one bought by a past finding — and `.speccle/remedies.jsonl` is why a finding that recurs gets the same answer twice.
...
**Planning settles where the work lands.** Every plan picks a **delivery route** and names it in the plan summary: **main** — the default, where commits land on `main` and `/review` closes the loop — or **pull request**, where the work rides a branch and the same lenses run again in CI. Opt in when a change is worth a second pair of eyes: cross-cutting, on shared ground, or the first of its kind. On that route the session that commits the contract cuts the branch first (`feature/<key-lowercased>-<slug>`), so from then on `git branch --show-current` is the record and no later session needs telling twice. Criteria commit there as usual; when `next` reports the slice done, `/review` runs locally exactly as it always does, and then the branch goes up and the pull request opens — the plan-time opt-in is the authorisation, so there is no second ask. CI's lenses post on the pull request and `/address` clears what they found. The `delivery-route` plan lens (`.speccle/lenses/plan/`) holds every plan to naming its route.
